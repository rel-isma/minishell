...
/*   Updated: 2023/08/12 13:48:11 by rel-isma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

void	ft_exec_in_child(t_list *cmd, char *path, char **env, int *fd,
		int old_fd)
{
	...
	{
		if ((ft_strncmp("/tmp/.heredoc>", (tl(cmd->content))->infilename,
					14) == 0))
			(tl(cmd->content))->infile = open((tl(cmd->content))->infilename,
				O_RDONLY);
...
int	ft_exec_cmd(t_list *cmd, int *fd, int old_fd)
{
	...
	if ((tl(cmd->content))->argms[0] && (tl(cmd->content))->argms[0][0] != '\0'
		&& (tl(cmd->content))->infile != -1)
	{
		if (ft_strcmp((tl(cmd->content))->cmd, "..") == 0
			|| ft_strcmp((tl(cmd->content))->cmd, ".") == 0)
		{
			command_not_found((tl(cmd->content))->cmd);
			g_minishell.exit_code = 127;
			return (0);
